...
if (!$selectedCategory && $applyFilters) {
    ...
    $sql = "SELECT c.id, c.name, c.image, c.description FROM categories c WHERE c.parent_id = 14 $filterWhere ORDER BY c.name";
    ...
    $stmt = $conn->prepare("SELECT id, name, image, description FROM categories WHERE parent_id = 14 ORDER BY name");
    $stmt->execute();
    $res = $stmt->get_result();
    while ($row = $res->fetch_assoc()) {
        $filteredCategories[] = $row;
...
if (!$selectedCategory) {
    $stmt = $conn->prepare("
        ...
        WHERE cf.category_id BETWEEN 14 AND 16
...
<head>
    ...
    <title>Exel Holdings - <?= $selectedCategory ? htmlspecialchars($categoryData['name'] ?? 'Category') : 'Engineered Stones' ?></title>
...
<section class="w-full bg-center relative pt-20 pb-20 sm:pt-24 sm:pb-24 px-4 sm:px-6 lg:px-12 2xl:px-16 header-section"
    ...
    <div class="relative z-10 text-center text-white max-w-3xl sm:max-w-4xl 2xl:max-w-5xl mx-auto">
        <h1 class="text-2xl sm:text-4xl md:text-5xl lg:text-6xl 2xl:text-7xl font-bold mb-4">
            <?= $selectedCategory ? htmlspecialchars($categoryData['name'] ?? 'Category') : 'Engineered Stones' ?>
        </h1>
        <nav class="text-xs sm:text-sm md:text-base 2xl:text-lg space-x-2">
            ...
            <a href="natural-stones.php" class="hover:underline text-white/80">Engineered Stones</a>
