...
function getCategoryChildren($conn, $parentId) {
    $children = [];
...
function getCategoryChildrenCount($conn, $categoryId) {
    $stmt = $conn->prepare('SELECT COUNT(*) as count FROM `jaquar-categories` WHERE parent_id = ?');
    if (!$stmt) return 0;
...
if ($categoryFilter != 0) {
    $stmt = $conn->prepare('SELECT id, name, image, parent_id FROM `jaquar-categories` WHERE id = ? LIMIT 1');
    if ($stmt) {
        ...
        if ($result->num_rows > 0) {
            ...
            if (!empty($currentCategory['image'])) {
                $categoryImage = htmlspecialchars($currentCategory['image']);
            }
        }
        $stmt->close();
    }
...
if ($categoryFilter == 0) {
    $showCategories = getCategoryChildren($conn, 1); // Show SANITARYWARE subcategories
} elseif ($currentCategory) {
    if (getCategoryChildrenCount($conn, $categoryFilter) > 0) {
        $showCategories = getCategoryChildren($conn, $categoryFilter); // Show sub-subcategories
    } else {
...
if ($showProducts) {
    ...
    if ($stmt) {
        ...
        $stmt->execute();
        $result = $stmt->get_result();
        $totalProducts = (int) $result->fetch_assoc()['total'];
        $stmt->close();
...
// Build breadcrumbs
$breadcrumbs = [['name' => 'Home', 'url' => 'index.php'], ['name' => 'Sanitaryware', 'url' => '?category=0']];
if ($currentCategory) {
    $path = [];
    ...
    while ($currentId) {
        ...
        if ($stmt) {
            ...
            if ($row = $result->fetch_assoc()) {
                $path[] = ['name' => $row['name'], 'url' => '?category=' . $row['id']];
...
<head>
    ...
    <title>Exel Holdings - Sanitaryware</title>
    ...
    <style>
        @media (min-width: 1536px) {
            ...
            .product-item img { height: 20rem; }
            .product-item h3 { font-size: 1.25rem; }
            .category-grid { grid-template-columns: repeat(4, minmax(0, 1fr)); }
            .category-item img { height: 16rem; }
            .pagination a { padding: 0.75rem 1.25rem; }
        }
        .loading-skeleton {
    ...
    <section class="py-10 sm:py-16 2xl:py-20 bg-white">
        <div class="container max-w-7xl 2xl:max-w-[1536px] mx-auto px-4 sm:px-6 lg:px-8 2xl:px-12">
            ...
            <?php if (!empty($showCategories)): ?>
                <!-- Display Categories -->
                <div class="category-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-4 gap-6 2xl:gap-8">
                    <?php foreach ($showCategories as $cat): ?>
                        <?php
                        $img = !empty($cat['image']) ? htmlspecialchars($cat['image']) : './assets/img/placeholder.png';
                        $hasChildren = getCategoryChildrenCount($conn, $cat['id']) > 0;
                        $buttonText = $hasChildren ? 'View Subcategories' : 'View Products';
                        ?>
            ...
            <?php elseif ($showProducts): ?>
                <!-- Display Products -->
                <div class="product-grid grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 2xl:gap-8">
                    <?php if (!empty($products)): ?>
                        <?php foreach ($products as $product): ?>
                            <?php
                            $img = !empty($product['image']) ? htmlspecialchars($product['image']) : './assets/img/bg-about.jpg';
                            ?>
                            <div class="product-item bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500">
                                <div class="relative overflow-hidden group">
                                ...
                                <div class="p-5 2xl:p-6">
                                    ...
                                    <a href="jaquar-product-details.php?id=<?= $product['id'] ?>"
                    ...
                    <?php else: ?>
                        <div class="col-span-full text-center py-16">
                            <div class="max-w-md mx-auto">
                                ...
                                <a href="?category=0" class="inline-flex items-center px-4 py-2 bg-yellow-400 text-black font-semibold rounded-lg hover:bg-yellow-500 transition-colors">
                ...
                <?php if ($totalPages > 1): ?>
                    <div class="mt-8 2xl:mt-12 flex justify-center space-x-2 pagination">
                        <?php if ($page > 1): ?>
                            <a href="?category=<?= $categoryFilter ?>&page=<?= $page - 1 ?>" class="px-4 py-2 2xl:px-5 2xl:py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">« Prev</a>
                        <?php endif; ?>
                        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                            <a href="?category=<?= $categoryFilter ?>&page=<?= $i ?>" class="px-4 py-2 2xl:px-5 2xl:py-3 rounded-lg <?= ($i == $page) ? 'bg-yellow-400 text-black font-bold' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' ?>"><?= $i ?></a>
                        <?php endfor; ?>
                        <?php if ($page < $totalPages): ?>
                            <a href="?category=<?= $categoryFilter ?>&page=<?= $page + 1 ?>" class="px-4 py-2 2xl:px-5 2xl:py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">Next »</a>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
            <?php else: ?>
                <div class="text-center py-16">
                    <h3 class="text-xl 2xl:text-2xl font-bold text-gray-900 mb-2">Nothing Found</h3>
                    <a href="?category=0" class="inline-flex items-center px-4 py-2 bg-yellow-400 text-black font-semibold rounded-lg hover:bg-yellow-500 transition-colors">
                        <i class="fas fa-arrow-left mr-2"></i>
                        Browse All Categories
                    </a>
